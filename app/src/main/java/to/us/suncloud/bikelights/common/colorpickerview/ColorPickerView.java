...
import to.us.suncloud.bikelights.common.colorpickerview.flag.FlagMode;
...
import static to.us.suncloud.bikelights.common.colorpickerview.ColorPickerDialog.SAVED_COLORS;
...
public class ColorPickerView extends FrameLayout implements LifecycleObserver {
    ...
    public void saveCurrentColorToFile() {
        // This method will save the colorPickerView's current color to the SAVED_COLORS file, so that it can be easily accessed later
        // Get the information from the colorPickerView

        Point point = getSelectedPoint();
//        int pointX = (int) getSelectorX();
//        int pointY = (int) getSelectorY();
        int color = getColor();
        int brightnessVal = getBrightnessSlider().getSelectedX();

        // Construct a SavedColor object from this information
        SavedColor savedColor = new SavedColor(point.x, point.y, color, brightnessVal);

        // Read the current array of saved colors, if it exists
        ArrayList<SavedColor> savedColorsList = (ArrayList<SavedColor>) LocalPersistence.readObjectFromFile(getContext(), SAVED_COLORS);
        if (savedColorsList == null) {
            // If there are no savedColor's in the file at the moment, create a new ArrayList for them
            savedColorsList = new ArrayList<>();
        }

        // If the list already contains the color, then remove it so that it can be "moved" to the beginning of the list
        savedColorsList.remove(savedColor);

        // Append the newly created SavedColor to the savedColorsList, but make sure that it is first
        savedColorsList.add(0, savedColor);

        // Finally, limit the size of the savedColorsList to 16
        int sizeLim = ColorPickerDialog.numColorBoxesPerRow*ColorPickerDialog.numColorBoxRows;
        if (savedColorsList.size() > sizeLim) {
            savedColorsList.subList(sizeLim, savedColorsList.size()).clear();
        }

        // Save the newly appended savedColorsList to file
        LocalPersistence.writeObjectToFile(getContext(), savedColorsList, SAVED_COLORS);
    }
}